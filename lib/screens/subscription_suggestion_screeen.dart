...
class SubscriptionSuggestionScreen extends StatefulWidget {
  ...
  @override
  State<SubscriptionSuggestionScreen> createState() => _SubscriptionSuggestionScreenState();
}

class _SubscriptionSuggestionScreenState extends State<SubscriptionSuggestionScreen> {
  ...
  final List<String> _frequencyTitles = ['Daily', 'Weekly', 'Bi-weekly', 'Monthly'];
  ...
  Future<void> _generateSuggestions() async {
    ...
    try {
      ...
      final suggestions = await _getChatGPTSuggestions(purchaseHistory);
  ...
  Future<Map<String, List<ProductSuggestion>>> _getChatGPTSuggestions(
    ...
    Consider purchase frequency and quantity patterns in the history.
    ''';
  ...
  Future<void> _addProduct(String frequency) async {
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        final product = Product.fromMap(productData); // Assuming Product.fromMap exists
        setState(() {
                    ...
                    Padding(
                      ...
                      child: Column(
                        children: [
                            ...
                            itemBuilder: (context, suggestionIndex) {
                              ...
                              return StatefulBuilder(
                                builder: (context, setState) {
                                  Variety? selectedVariety = suggestion.product?.varieties.isNotEmpty ?? false
                                      ? suggestion.product!.varieties.first
                                      : null;

                                  return ExpansionTile(
                                    leading: _buildLeadingImage(suggestion, selectedVariety),
                                    ...
                                    subtitle: Row(
                                      children: [
                                        ...
                                        IconButton(
                                          ...
                                          onPressed: () => setState(() => _updateQuantity(frequency, suggestionIndex, suggestion.quantity - 1)),
                                        ),
                                        IconButton(
                                          icon: const Icon(Icons.add),
                                          onPressed: () => setState(() => _updateQuantity(frequency, suggestionIndex, suggestion.quantity + 1)),
                                        ),
                                        Text('Price: \$${_getPrice(suggestion, selectedVariety)}'),
                                      ],
                                    ),
                                    trailing: IconButton(
                                      icon: const Icon(Icons.remove_circle),
                                      onPressed: () => _removeProduct(frequency, suggestionIndex),
                                    ),
                                    children: [
                                      if (suggestion.product?.varieties.isNotEmpty ?? false)
                                        SizedBox(
                                          height: 150,
                                          child: ListView.builder(
                                            scrollDirection: Axis.horizontal,
                                            itemCount: suggestion.product!.varieties.length,
                                            itemBuilder: (context, varietyIndex) {
                                              final variety = suggestion.product!.varieties[varietyIndex];
                                              return Padding(
                                                padding: const EdgeInsets.all(8.0),
                                                child: GestureDetector(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    decoration: BoxDecoration(
                                                      border: Border.all(
                                                        color: selectedVariety == variety ? Colors.green : Colors.grey,
                                                      ),
                          ...
                          const SizedBox(height: 8),
                          ElevatedButton(
                            onPressed: () => _subscribeToFrequency(frequency, _frequencies[index]),
  ...
  Widget? _buildLeadingImage(ProductSuggestion suggestion, Variety? selectedVariety) {
    if (selectedVariety?.imageUrl != null) {
      return Image.network(
        ...
        height: 40,
        fit: BoxFit.cover,
        errorBuilder: (context, error, stackTrace) => const Icon(Icons.error_outline, size: 40),
      );
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Subscribe to all ${suggestions.length} products in $frequency?'),
            ...
            TextButton(
              onPressed: () {
                ...
                for (var suggestion in suggestions) {
                  ...
                  final selectedVariety = suggestion.product?.varieties.isNotEmpty ?? false
                      ? suggestion.product!.varieties.first
                      : null;
                  ...
                  final subscription = Subscription(
                    ...
                    price: selectedVariety?.price ?? suggestion.product!.basePrice,
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Subscribed to $frequency suggestions')),
                );
              },
  ...
  Widget _buildFrequencyDropdown(int selectedFrequency, List<int> availableFrequencies, void Function(void Function()) setState) {
    return Column(
  ...
  Widget _buildDayDropdown(int selectedDay, List<String> weekDays, void Function(void Function()) setState) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SizedBox(height: 16),
