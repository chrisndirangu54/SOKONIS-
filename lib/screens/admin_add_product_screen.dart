...
class AdminAddProductScreenState extends State<AdminAddProductScreen> {
  ...
  late final GroupBuyService?
      groupBuyService; // Define the groupBuyService variable
  ...
  Future<void> _loadProductDetails() async {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        _complementaryProductIdsController.text =
            (data?['complementaryProductIds'] ?? []).join(', ');
        _seasonStartController.text = data?['seasonStart']?.toString() ?? '';
        _seasonEndController.text = data?['seasonEnd']?.toString() ?? '';
        _consumptionTimeController.text =
            (data?['consumptionTime'] ?? []).join(', ');
        ...
        _groupDiscountController.text =
            (data?['groupDiscount'] ?? 0.0).toString();
        ...
        _genomicAlternativesController.text =
            (data?['genomicAlterations'] ?? []).join(', ');
        // Load varieties
    ...
    try {
      ...
      final productData = {
        ...
        'complementaryProductIds':
            _complementaryProductIdsController.text.split(', '),
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        for (var fileEntry in result.files) {
          ...
          List<Product> fileProducts =
              await _determineProductsUsingAIImplementation(
            productName,
            productData,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Added ${products.length} product(s) using AI!')),
      );
  ...
  Future<List<Product>> _determineProductsUsingAIImplementation(
    ...
    final url =
        Uri.parse('https://api.x.ai/v1/chat/completions'); // Verify endpoint
    String? grokApiKey;
    ...
    String query = 'Product: $productName. Data: ${jsonEncode(productData)}. '
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> consumptionTimeOptions = [
      'lunch',
      'breakfast',
      'supper'
    ];
    ...
    List<String> selectedConsumptionTimes =
        _consumptionTimeController.text.isNotEmpty
            ? _consumptionTimeController.text
                .split(',')
                .map((s) => s.trim())
                .toList()
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                        ...
                        decoration:
                            const InputDecoration(labelText: 'Picture URL')),
                    TextField(
                          ...
                          Wrap(
                            ...
                            children: consumptionTimeOptions.map((option) {
                              return FilterChip(
                                label: Text(option),
                                selected:
                                    selectedConsumptionTimes.contains(option),
                                onSelected: (selected) {
                                  setState(() {
                                    if (selected) {
                    ...
                    TextField(
                        ...
                        decoration:
                            const InputDecoration(labelText: 'Group Discount'),
                        keyboardType: TextInputType.number),
                    TextField(
                    ...
                    ..._varietyControllers.map((vc) => Column(
                          children: [
                            ...
                            TextField(
                                controller: vc['color'],
                                decoration: const InputDecoration(
                                    labelText: 'Variety Color')),
                            TextField(
                                controller: vc['size'],
                                decoration: const InputDecoration(
                                    labelText: 'Variety Size')),
                            TextField(
                                controller: vc['imageUrl'],
                                decoration: const InputDecoration(
                                    labelText: 'Variety Image URL')),
                            TextField(
                                controller: vc['price'],
                                decoration: const InputDecoration(
                                    labelText: 'Variety Price'),
                                keyboardType: TextInputType.number),
                            TextField(
                                controller: vc['discountedPrice'],
                                decoration: const InputDecoration(
                                    labelText: 'Variety Discounted Price'),
                                keyboardType: TextInputType.number),
                    ...
                    const SizedBox(height: 20),
                    _buildCheckbox('Is Fresh', _isFresh,
                        (value) => setState(() => _isFresh = value ?? false)),
                    _buildCheckbox(
                        'Is Locally Sourced',
                        _isLocallySourced,
                        (value) =>
                            setState(() => _isLocallySourced = value ?? false)),
                    _buildCheckbox('Is Organic', _isOrganic,
                        (value) => setState(() => _isOrganic = value ?? false)),
                    _buildCheckbox(
                        'Has Health Benefits',
                        _hasHealthBenefits,
                        (value) => setState(
                    ...
                    _buildCheckbox(
                        ...
                        _hasDiscounts,
                        (value) =>
                            setState(() => _hasDiscounts = value ?? false)),
                    _buildCheckbox(
                        'Is Eco-Friendly',
                        _isEcoFriendly,
                        (value) =>
                            setState(() => _isEcoFriendly = value ?? false)),
                    _buildCheckbox(
                        'Is Superfood',
                        _isSuperfood,
                        (value) =>
                            setState(() => _isSuperfood = value ?? false)),
                    _buildCheckbox(
                        'Is Complementary',
                        _isComplementary,
                        (value) =>
                            setState(() => _isComplementary = value ?? false)),
                    _buildCheckbox(
                        'Is Seasonal',
                        _isSeasonal,
                        (value) =>
                    ...
                    _buildCheckbox(
                        ...
                        _isGroupActive,
                        (value) =>
                            setState(() => _isGroupActive = value ?? false)),
                    ...
                    ElevatedButton(
                        ...
                        child: const Text('Add Product Options')),
                  ],
                ),
              ),
            ),
